...
class MyAlignment:
	def __init__(self, row):
		...
		self.mismatches		= int(row[5])
		self.gapopen 		= int(row[6]) 				# mismatches
		self.gaps 			= int(row[7])				# gap openings
		self.qstart 		= int(row[8])				# query start
		self.qend 			= int(row[9])				# query end
		self.sstart 		= int(row[10])				# subject start
		self.send 			= int(row[11]) 				# subject end
